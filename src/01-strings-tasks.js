...
//  throw new Error('Not implemented');
  const numSpace = value.indexOf(' ');
  return value.substring(numSpace + 1, value.length - 1);
}
...
//  throw new Error('Not implemented');
  ...
  const angleLeftUp = '┌';
  const angleLeftDown = '└';
  const angleRightDown = '┘';
  const lineVertical = '│';
  const lineHorizontal = '─';
  const rectangleUp = `${angleLeftUp}${lineHorizontal.repeat(width - 2)}${angleRightUp}\n`;
  const rectangleDown = `${angleLeftDown}${lineHorizontal.repeat(width - 2)}${angleRightDown}\n`;
  const rectangleMiddle = `${lineVertical}${' '.repeat(width - 2)}${lineVertical}\n`;
  for (let i = 0; i < height; i += 1) {
    if (i === 0) {
      rectangle = rectangleUp;
    }
    if (i > 0 && i < height - 1) {
      rectangle += rectangleMiddle;
    }
    if (i === height - 1) {

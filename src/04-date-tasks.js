...
//  throw new Error('Not implemented');
  const date = new Date(endDate.getTime() - startDate.getTime()).toISOString();
...
//  throw new Error('Not implemented');
  ...
  let angleHour = Math.abs(0.5 * (60 * hour + minute));
  let angleMinute = 6 * minute;
  if (angleHour > 180) {
    angleHour = Math.abs(360 - angleHour);
  }
  if (angleMinute > 180) {
    angleMinute = Math.abs(360 - angleMinute);
  }
  let angle = angleHour - angleMinute;
